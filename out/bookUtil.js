"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
const vscode_1 = require("vscode");
const fs = require("fs");
class Book {
    constructor(extensionContext) {
        this.curr_page_number = 1;
        this.page_size = 50;
        this.page = 0;
        this.start = 0;
        this.end = this.page_size;
        this.filePath = "";
        this.extensionContext = extensionContext;
    }
    getFileKey() {
        if (!this.filePath) {
            return "";
        }
        return this.filePath.toLowerCase();
    }
    getPageStore() {
        const store = this.extensionContext.globalState.get("thiefBook.pageByFilePath", {});
        if (store && typeof store === "object") {
            return store;
        }
        return {};
    }
    getSavedPageForCurrentFile() {
        const key = this.getFileKey();
        if (!key) {
            // 没有设置文件路径时，兼容旧的全局页码配置
            const fallback = vscode_1.workspace.getConfiguration().get('thiefBook.currPageNumber');
            return (typeof fallback === "number" && fallback >= 1) ? fallback : 1;
        }
        const store = this.getPageStore();
        const saved = store[key];
        if (typeof saved === "number" && saved >= 1) {
            return saved;
        }
        // 该小说从未打开过，从第 1 页开始
        return 1;
    }
    clampPage(page) {
        if (typeof page !== "number" || Number.isNaN(page)) {
            return 1;
        }
        if (page < 1) {
            return 1;
        }
        if (page > this.page) {
            return this.page;
        }
        return page;
    }
    getSize(text) {
        let size = text.length;
        this.page = Math.ceil(size / this.page_size);
    }
    getFileName() {
        var file_name = this.filePath.split("/").pop();
        console.log(file_name);
    }
    getPage(type) {
        var curr_page = this.curr_page_number;
        var page = 0;
        if (type === "previous") {
            if (curr_page <= 1) {
                page = 1;
            }
            else {
                page = curr_page - 1;
            }
        }
        else if (type === "next") {
            if (curr_page >= this.page) {
                page = this.page;
            }
            else {
                page = curr_page + 1;
            }
        }
        else if (type === "curr") {
            page = curr_page;
        }
        this.curr_page_number = page;
        // this.curr_page_number = this.extensionContext.globalState.get("book_page_number", 1);
    }
    updatePage() {
        // var page = 0;
        // if (type === "previous") {
        //     if (this.curr_page_number! <= 1) {
        //         page = 1;
        //     } else {
        //         page = this.curr_page_number! - 1;
        //     }
        // } else if (type === "next") {
        //     if (this.curr_page_number! >= this.page) {
        //         page = this.page;
        //     } else {
        //         page = this.curr_page_number! + 1;
        //     }
        // }
        vscode_1.workspace.getConfiguration().update('thiefBook.currPageNumber', this.curr_page_number, true);
        const key = this.getFileKey();
        if (key) {
            const store = this.getPageStore();
            store[key] = this.curr_page_number;
            this.extensionContext.globalState.update("thiefBook.pageByFilePath", store);
        }
        // this.extensionContext.globalState.update("book_page_number", page);
    }
    getStartEnd() {
        this.start = this.curr_page_number * this.page_size;
        this.end = this.curr_page_number * this.page_size - this.page_size;
    }
    readFile() {
        if (this.filePath === "" || typeof (this.filePath) === "undefined") {
            vscode_1.window.showWarningMessage("请填写TXT格式的小说文件路径 & Please fill in the path of the TXT format novel file");
        }
        var data = fs.readFileSync(this.filePath, 'utf-8');
        var line_break = vscode_1.workspace.getConfiguration().get('thiefBook.lineBreak');
        return data.toString().replace(/\n/g, line_break).replace(/\r/g, " ").replace(/　　/g, " ").replace(/ /g, " ");
    }
    init() {
        this.filePath = vscode_1.workspace.getConfiguration().get('thiefBook.filePath');
        var is_english = vscode_1.workspace.getConfiguration().get('thiefBook.isEnglish');
        if (is_english === true) {
            this.page_size = vscode_1.workspace.getConfiguration().get('thiefBook.pageSize') * 2;
        }
        else {
            this.page_size = vscode_1.workspace.getConfiguration().get('thiefBook.pageSize');
        }
        this.curr_page_number = this.getSavedPageForCurrentFile();
    }
    getPreviousPage() {
        this.init();
        let text = this.readFile();
        this.getSize(text);
        this.getPage("previous");
        this.getStartEnd();
        var page_info = this.curr_page_number.toString() + "/" + this.page.toString();
        this.updatePage();
        return text.substring(this.start, this.end) + "    " + page_info;
    }
    getNextPage() {
        this.init();
        let text = this.readFile();
        this.getSize(text);
        this.getPage("next");
        this.getStartEnd();
        var page_info = this.curr_page_number.toString() + "/" + this.page.toString();
        this.updatePage();
        return text.substring(this.start, this.end) + "    " + page_info;
    }
    getJumpingPage() {
        this.init();
        let text = this.readFile();
        this.getSize(text);
        this.curr_page_number = this.clampPage(vscode_1.workspace.getConfiguration().get('thiefBook.currPageNumber'));
        this.getStartEnd();
        var page_info = this.curr_page_number.toString() + "/" + this.page.toString();
        this.updatePage();
        return text.substring(this.start, this.end) + "    " + page_info;
    }
}
exports.Book = Book;
//# sourceMappingURL=bookUtil.js.map